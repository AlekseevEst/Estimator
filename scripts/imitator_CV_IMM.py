...
R = np.diag([10000.0, (0.1/3.0)**2,(0.1/3.0)**2]) #дисперсии, в deg
...
X_true_plus_ProcNoise_not_pass = X_true_plus_ProcNoise_with_pass[:, ~pass_index] # вырезаю пропуски, чтобы построить график
...
tg = Target()
tg.init_state(init_state)
X_true_data_not_pass = make_true(tg,n)
std_err = calc_std_err(X_true_data_not_pass)


plt.figure(num="ImmCV")
plt.subplot(6, 1, 1)
