...
def calc_std_err(X):
    num_iterations = 1
    var_err = np.zeros((X.shape[0]-1, X.shape[1]-1)) ## изменения так как модель CV. а настроено было под CT (моделируется CT. обрезал последний w)
